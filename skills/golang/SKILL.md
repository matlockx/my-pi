...
**Always use Go 1.27+.** Never target or reference older versions (1.22, 1.23, 1.24, etc.) in generated code, `go.mod`, or configs.
...
run:
  timeout: 5m
  go: "1.27"  # minimum — never target older versions
linters:
  default: standard
  enable:
    - gosec
